...
def from_mmpkl_det(
    ...
    dataset = Dataset(frames=frames[:100], groups=groups, config=cfg)
    return dataset
...
def from_mmpkl(
    ...
    if data_type == "detection":
        return from_mmpkl_det(input_dir, data_dir, samples_list, data_type, class_name, eval_type, class_difficulty,iou, metric, order)
...
def run(args: argparse.Namespace) -> None:
    ...
    scalabel = from_mmpkl(
        ...
        class_name=args.name, eval_type=args.eval_type, class_difficulty=args.difficulty, iou=args.iou, metric=args.metric, order=args.order)
